...
static Ciphertext bce_with_logits_grad_poly(Ciphertext &ct_logits,  // [B,1] in first block
                                            ...
                                            double target_scale) {
    // sigmoid(logit)
    Ciphertext sig = sigmoid_poly3(ct_logits, encoder, evaluator, relin_keys, context);
...
static NetCache net_forward_demo(SEALContext &context,
    ...
    // demo dims (match your style)
    size_t B = 8;
    size_t k = 1024;    // for bopa inner width
    size_t k_in = 2048; // concat gives 2 branches -> roughly doubles; we demo as 2048
    size_t h = 1024;
...
static NetGrads net_backward_demo(const NetCache &cache,
    ...
    g.g1 = bopa_backward(cache.bopa1, g.d_bopa1_out, evaluator, gal_keys, relin_keys, context, target_scale);
    g.g2 = bopa_backward(cache.bopa2, g.d_bopa2_out, evaluator, gal_keys, relin_keys, context, target_scale);
...
int main() {
    ...
    double target_scale = pow(2.0, 50);

    cout << "[init] slot_count=" << encoder.slot_count() << "\n";

    // ---- forward ----
    NetCache cache = net_forward_demo(context, encoder, encryptor, evaluator, gk, rk, scale, target_scale);
    cout << "[forward] states:\n";
    ...
    // ---- backward ----
    cout << "\n[backward] running...\n";
    NetGrads grads = net_backward_demo(cache, y, context, encoder, evaluator, gk, rk, target_scale);
